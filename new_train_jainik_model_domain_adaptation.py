...
from new_resnet_Jainik_model_domain_adaptation import ResNet1d 
...
def compute_loss_domain(domain_predictions, domain_labels):
    ...
    domain_loss = criterion_domain(domain_predictions, domain_labels)
    return domain_loss
...
# transforming data and doing test train validation split of source domain
    ...
    num_samples_to_keep = 20000  # Set the desired number of samples to keep
...
# loading and transforming data and test train split of target domain
    ...
    source_dataloader = DataLoader(source_domain_train, batch_size=args.batch_size, shuffle=True)
    target_dataloader = DataLoader(target_domain_train, batch_size=args.batch_size, shuffle=True)
    ...
    for source_batch, target_batch in zip(source_dataloader, target_dataloader):
        ...
        source_domain_labels = torch.zeros(X_source.shape[0])  # Source domain labels
        target_domain_labels = torch.ones(X_target.shape[0])  # Target domain labels

        combined_X = torch.cat((X_source, X_target), dim=0)
        combined_y = y_source
        combined_domain_labels = torch.cat((source_domain_labels, target_domain_labels), dim=0)

        combined_batch = (combined_X, combined_y, combined_domain_labels)
        combined_dataloader.append(combined_batch)

print("training started")
for epoch in range(epochs):
    train_loss = train(epoch, combined_dataloader)
    print(train_loss)
        #valid_MAE = eval(epochs, dl_test)
